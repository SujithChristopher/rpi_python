...
class AprilTagBoard:
    """Represents the 3D printed board with 3 AprilTags in equilateral triangle configuration"""
    
    def __init__(self, tag_ids=[12, 14, 20], triangle_side_length=200.0, marker_size=50.0):
        """
        Initialize AprilTag board configuration
        
        Args:
            tag_ids: List of AprilTag IDs
            triangle_side_length: Distance between tag centers in mm
        ...
        # Assuming equilateral triangle with center at origin
        height = triangle_side_length * np.sqrt(3) / 2
        self.tag_positions_3d = {
            tag_ids[0]: np.array([0, 2*height/3, 0]),  # Top
            tag_ids[1]: np.array([-triangle_side_length/2, -height/3, 0]),  # Bottom left
            tag_ids[2]: np.array([triangle_side_length/2, -height/3, 0])   # Bottom right
        }
        
        # Store relative constraints
        self.expected_distances = {}
        for i, id1 in enumerate(tag_ids):
            for j, id2 in enumerate(tag_ids[i+1:], i+1):
                key = tuple(sorted([id1, id2]))
                self.expected_distances[key] = np.linalg.norm(
                    self.tag_positions_3d[id1] - self.tag_positions_3d[id2]
                )
            ...
            if len(tag_centers_2d) >= 2:
                for i in range(len(tag_centers_2d)):
                    for j in range(i+1, len(tag_centers_2d)):
                        ...
                        expected_ratio = dist_3d / april_board.triangle_side
                        if valid_frames > 0:
...
class IterativeFisheyeCalibrator:
    ...
    def refine_calibration(self, april_detections, max_iterations=10, tolerance=0.01):
        ...
        # Initial parameters - ensure proper array extraction
        fx = float(self.camera_matrix[0, 0])
        fy = float(self.camera_matrix[1, 1])
        cx = float(self.camera_matrix[0, 2])
        cy = float(self.camera_matrix[1, 2])
        
        # Handle distortion coefficients properly
        if isinstance(self.dist_coeffs, np.ndarray):
            dist_flat = self.dist_coeffs.flatten()
        else:
            dist_flat = np.array(self.dist_coeffs).flatten()
        
        # Ensure we have exactly 4 distortion coefficients
        if len(dist_flat) < 4:
            # Pad with zeros if needed
            dist_flat = np.pad(dist_flat, (0, 4 - len(dist_flat)), 'constant')
        
        k1 = float(dist_flat[0])
        k2 = float(dist_flat[1])
        k3 = float(dist_flat[2])
        k4 = float(dist_flat[3])
        ...
        result = differential_evolution(
            ...
            tol=tolerance,
            seed=42,
            workers=1,  # Use single worker to avoid pickling issues with cv2 objects
            updating='deferred',
            disp=True
        )
...
def main():
    ...
    april_board = AprilTagBoard(
        tag_ids=[12, 14, 20],
        triangle_side_length=200.0,  # Adjust based on your actual board
    ...
    try:
        # Try the differential evolution first
        success = calibrator.refine_calibration(april_detections, max_iterations=50)
    except Exception as e:
        ...
        success = calibrator.refine_calibration_simple(april_detections, max_iterations=100)
