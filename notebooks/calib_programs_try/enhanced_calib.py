...
class EnhancedFisheyeCalibration:
    ...
    def calibrate_single_iteration(self, chessb_corners, n_samples=20):
        ...
        if self.config.use_adaptive_selection:
            indices = self.select_diverse_samples(chessb_corners, n_samples)
        else:
            indices = np.random.choice(len(chessb_corners), min(n_samples, len(chessb_corners)), replace=False)
        ...
        if self.config.use_multi_scale:
            regions, region_indices = self.classify_corners_by_region(chessb_c)
            ...
            for i, corners in enumerate(chessb_c):
                ...
                if dist_from_center < min(h, w) * 0.3:
                    weights[i] = self.config.multi_scale_weights[0]
                elif dist_from_center < min(h, w) * 0.6:
                    weights[i] = self.config.multi_scale_weights[1]
                else:
                    weights[i] = self.config.multi_scale_weights[2]
        ...
        try:
            ...
            if self.config.use_bundle_adjustment:
                result = self.refine_with_bundle_adjustment(result, chessb_c)
    ...
    def generate_calibrations(self, chessb_corners, n_calibrations=200, n_jobs=20):
        ...
        results = Parallel(n_jobs=n_jobs)(
            delayed(self.calibrate_single_iteration)(chessb_corners, n_samples=20) 
...
def main():
    ...
    config = CalibrationConfig(
        ...
        use_temperature_selection=False,  # Simulated annealing selection
        cache_undistortion_maps=True,     # Cache for performance
        use_adaptive_aruco=False,         # Adaptive ArUco detection
        use_fisheye_refinement=False,     # Fisheye-specific corner refinement
    ...
    april_data_path = os.path.join(base_path, "data", 'recordings', "160_fov", "3marker_complete_data", "randomxyz_t0", "webcam_color.msgpack")
    ...
    n_calibrations = 100 if config.use_bundle_adjustment else 200  # Fewer if using slow methods
